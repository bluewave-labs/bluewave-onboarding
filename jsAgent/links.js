...
bw.links={
    ...
    putIcon:function(){
        let temp_html = `
        <div id="bw-links" style="position: fixed; bottom: 20px; right: 30px; z-index: 99; border: none; outline: none; cursor: pointer; display: flex; flex-direction: column; align-items: flex-end; justify-content: space-evenly; flex-wrap: nowrap; ">
    ...
    putPlaceHolder: function(){
        ...
        const temp_html = `<div id="bw-links-placeholder" style=" flex-direction: column; justify-content: space-between; display:none; position: width: 330px;  fixed; bottom: 70px; right: 30px; z-index: 99; border: none; outline: none; cursor: pointer;">

            ${bw.links.generateHeader(option.headerBackgroundColor)}
        
        </div>`;
      
        document.getElementById('bw-link-icon').insertAdjacentHTML('beforebegin', temp_html);
    },

    generateHeader: function(bgColor){
        const temp_header_html =`<div style="background-color:${bgColor}"> <div>`;
    },
    bindClick : function(){
    ...
    show : function(){
        document.getElementById('bw-links-placeholder').style.display = 'flex';
    },
    hide : function(){
